...
void tracker_fusor::startStandalone()
{
    ros::Rate rate(30.0);
    while(node_.ok()){
        runtime++;
...
void tracker_fusor::checkSkeletonsUpToDate()
{
...
double tracker_fusor::vectDist(Eigen::Vector3d a, Eigen::Vector3d b)
{
    Eigen::Vector3d c = a-b;
    return c.norm();
}

void tracker_fusor::mergeSkeletons()
{
...
KinectPacket* tracker_fusor::getFusedKinectPacket(){return kinpack_fused_;}
int tracker_fusor::getNumActiveKinects(){return num_active_kinect_;}

void tracker_fusor::userCallback(const openni_msgs::UserArrayConstPtr &userArray_msg)
{
    // Get number of users
...
void tracker_fusor::rgbCallback(const sensor_msgs::ImageConstPtr &image){
...
void tracker_fusor::depthCallback(const sensor_msgs::ImageConstPtr &image){
...
bool tracker_fusor::strContains(std::string s, std::string a)
{
    size_t found;
    found = s.find(a);
...
int tracker_fusor::findNumKinect(std::string s)
{
    int num;
...
int tracker_fusor::findNumTracker(std::string s)
{
    int num;
...
cv::Mat tracker_fusor::convertDepthTo8BitImage(cv::Mat img)
{ 
    double min_range = .5;
...
// KinectSkeletonMap tracker_fusor::getSkelsWorld(){return skels_world_;}

// KinectSkeletonMap tracker_fusor::getSkels(int kinect_id){return skels_kinect_[kinect_id];}

// cv::Mat tracker_fusor::getDepthImageFused(){return depth_image_fused_;}

// cv::Mat tracker_fusor::getDepthImage(int kinect_id){return depth_images_[kinect_id];}

// cv::Mat tracker_fusor::getRGBImageFused(){return rgb_image_fused_;}

// cv::Mat tracker_fusor::getRGBImage(int kinect_id){return rgb_images_[kinect_id];}


// bool tracker_fusor::usersAvailable(){return users_available_;}
